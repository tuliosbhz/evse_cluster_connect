...
from ocpp_v201.opt_scheduler import calculate_optimized_charging_schedule
...
class CSMS(cp):
    ...
    def on_authorize(self,id_token,*args, **kwargs):
        return call_result.AuthorizePayload(id_token_info={'status':"Accepted"})
    ...
    async def after_notify_ev_charging_needs(self,charging_needs,evse_id,max_schedule_tuples):
        if self.ac_charging_parameters:
            ...
            self.departure_time = datetime.now() if not self.departure_time else datetime.fromtimestamp(float(self.departure_time))
