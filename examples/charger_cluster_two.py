...
from cluster_db import SessionLocal, Configuration, ScheduleTable, Simulation
import sqlalchemy

from datetime import datetime
from charger_parameters import ConfigParameters, ScheduleParameters, SimulationParameters
...
class Charger(SyncObj):
    ...
    def __init__(self, evse_id):
        ...
        self.__cluster_ids = {}
        self.simulation_parameters = self.get_my_sim_parameters_from_db(self.id)
        self.__cluster_sim_params = { self.id: self.simulation_parameters,}
    ...
    def get_schedule_params_from_db(self, evse_id):
        # Verifica se o plug está conectado
        db = SessionLocal()
        sched_row = db.query(ScheduleTable).filter(ScheduleTable.evse_id == evse_id).first()
        ...
        if sched_row:
            ...
            schedule = ScheduleParameters(**sched_data)
            db.commit()
            db.close()
            return schedule
        #TODO: Cannot return None
    ...
    def get_my_sim_parameters_from_db(self, evse_id):
        ...
        try:
            # Verifica se o plug está conectado
            db = SessionLocal()
            evse_row = db.query(Simulation).where(Simulation.evse_id == evse_id).first()
            ...
            self.simulation_parameters = SimulationParameters(**charger_data)
            db.commit()
            ...
            print(self.simulation_parameters)
    ...
    def set_schedule_params_on_db(self):
        # Verifica se o plug está conectado
        db = SessionLocal()
        sched_row = db.query(ScheduleTable).filter(ScheduleTable.evse_id == self.id).first()
    ...
    """
    @replicated
    def add_sim_params_to_cluster(self, evse_id, params:SimulationParameters):
        self.__cluster_sim_params[evse_id] = params
    
    @replicated
    def update_sim_params_to_cluster(self, evse_id, params:SimulationParameters):
    ...
    """
    @replicated
    def add_my_sched_params_to_cluster(self, evse_id ,params:ScheduleParameters):
        self.__sched_cluster_parameters[evse_id] = params
    """
    Adiciona os dados de scheduling criado pelo lider a todos os nós
    """
    @replicated
    def update_sched_params_on_cluster(self, evse_id:str, params:ScheduleParameters):
    ...
    def count_auth_chargers(self):
        i = 0
        if self.__cluster_sim_params:
            #Itera por todos os outros nós do grupo
    ...
    def calculate_schedule(self, max_power_cluster):
        ...
        db = SessionLocal()
        sched_row = db.query(ScheduleTable).filter(ScheduleTable.evse_id == self.id).order_by(ScheduleTable.register_id.desc()).first()
        if sched_row:
            sched_row.max_cluster_power = self.__sched_cluster_parameters[self.id].max_cluster_power
            sched_row.selected_max_power = self.__sched_cluster_parameters[self.id].selected_max_power
            sched_row.user_id = self.__sched_cluster_parameters[self.id].user_id
    ...
    try:
        while True:
            ...
            if my_address not in o.get_sched_cluster_params():
                o.add_my_sched_params_to_cluster(my_address, o.get_schedule_params_from_db(my_address), sync=True)
            ...
            if o.hasQuorum == False:
                #Repassa o valor para o cluster 
                o.simulation_parameters = o.get_my_sim_parameters_from_db(my_evse_id)
                sched_parameters = o.calculate_schedule(o.simulation_parameters.max_cluster_power)
                #Repassa o schedule para o cluster
                #Atualização dos dados do cluster baseado no calculo de replicação do sistema
                if my_evse_id in o.get_sched_cluster_params():
                    o.update_sched_params_on_cluster(
            ...
            o.schedule_parameters.selected_max_power = o.get_sched_cluster_params()[o.get_my_evse_id()].selected_max_power
        ...
        o.update_sim_params_to_cluster(evse_id=my_evse_id, 
                                       params=SimulationParameters(plug_status="not_plugged",
                                                                    authenticated=False,
                                                                    user_id=0,
